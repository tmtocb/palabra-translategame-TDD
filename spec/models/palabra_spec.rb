...
RSpec.describe Palabra, type: :model do
  ...
  describe '#associations' do
    ...
    it {
      is_expected.to have_many(:translations).through(:translations_association).source(:translated_palabra).dependent(:destroy)
    }
    it {
      is_expected.to have_many(:inverse_translations_association).class_name('Translation').with_foreign_key('translated_palabra_id')
    }
